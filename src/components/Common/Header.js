...
export default function Header() {
  ...
  // LocalStorage se user check karne ka common function
  const checkUser = () => {
    const storedUser = localStorage.getItem("user");
    if (storedUser) {
      setUser(JSON.parse(storedUser));
  ...
  useEffect(() => {
    // Pehli baar page load hone par check karo
    checkUser();

    // ✅ Jab signup.js se "authChange" event fire hoga (login/signup ke turant baad)
    // tab bina page reload kiye Header khud ko update kar lega
    window.addEventListener("authChange", checkUser);

    // ✅ Agar user kisi doosre tab me logout/login kare to bhi sync ho jaye
    ...
    <header className="fixed top-0 left-0 w-full z-50 px-4 md:px-12 py-6">
      <div className="max-w-7xl mx-auto bg-[#FFFFFFE0]/88 backdrop-blur-sm rounded-xl shadow-sm flex items-center justify-between px-5 md:px-8 py-4">
        ...
        <div className="flex items-center gap-5 text-gray-800">
          ...
          {/* CONDITIONAL HEADER - login/signup ho jaye to yahan user icon aa jayega */}
          {user ? (
            <div className="flex items-center gap-1 cursor-pointer">
              <Link href={"/profile"}>
                <FaRegUser size={18} />
